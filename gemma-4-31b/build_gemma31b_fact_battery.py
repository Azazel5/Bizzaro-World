#!/usr/bin/env python3
"""
Phase 1 — Tokenizer filter for google/gemma-4-31B.

Downloads only the tokenizer (~50 MB, no GPU needed) and filters the master
battery for single-token targets + equal-length prompt pairs.

Run this locally before triage_bf16.py (Phase 2, needs GPU).

Usage:
    HF_TOKEN=your_token python gemma-4-31b/build_gemma31b_fact_battery.py
"""
...
MODEL_ID_DEFAULT = "google/gemma-4-31B"


def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Filter the master battery (gemma-2b.json) for Gemma 4 31B "
    ...
    ap.add_argument(
        "--out-battery",
        type=Path,
        default=REPO_ROOT / "fact_battery" / "gemma-4-31b-tokfiltered.json",
    )
    ap.add_argument(
        "--report",
        type=Path,
        default=REPO_ROOT / "fact_battery" / "gemma-4-31b-tokfiltered.drop_report.json",
    )
